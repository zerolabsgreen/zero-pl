...
import {
  CreateOrderDto,
  PaymentPreferencesEnumType,
  ProtocolTypeEnumType,
...
export const useWizardPageEffects = () => {
  ...
  const { mutate, isLoading } = useOrdersControllerCreate({
    mutation: {
      onSuccess: () => {
      ...
      const preparedValues: CreateOrderDto = {
        ...
        items: mappingArrIterator.map((key) => {
          return {
            country:
              Countries.find(
                (country) => country.name === values[`country_${key}`]
              )?.code || '',
            minerId: values[`minerId_${key}`] || '',
            // for some reason throws an error that mapping can be null even though
            // it is null checked
            timeFrames: mapping.get(key)
            // @ts-ignore
            ? mapping.get(key).map((nestedId) => ({
                  start: (values[`startDate_${key}_${nestedId}`] as Dayjs)
                    .startOf('day')
                    .toISOString(),
                  end: (values[`endDate_${key}_${nestedId}`] as Dayjs)
                    .endOf('day')
                    .toISOString(),
                  energy: BigNumber.from(values[`energy_${key}_${nestedId}`])
